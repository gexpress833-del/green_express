    ...
    <section className="page-section min-h-screen bg-[#0b1220] text-white">
      <div className="container">
        <header className="mb-8">
          ...
          <p className="text-white/70 text-lg">Toutes les commandes soumises — vue admin</p>
        </header>
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <div
                      ...
                      className="card rounded-xl transition-shadow duration-300 p-6"
                    >
                      {/* En-tête : numéro, date, adresse, client, statut */}
                        ...
                        <div className="mb-5 p-4 bg-white/5 rounded-lg border border-white/10">
                          ...
                          <ul className="space-y-3 list-none pl-0 m-0">
                            {order.items.map((item, idx) => {
                              ...
                              const itemCurrency = (item.menu?.currency || 'CDF').toUpperCase()
                              ...
                              const fmt = (n) => (itemCurrency === 'CDF' || itemCurrency === 'FC') ? formatCurrencyCDF(n) : `${Number(n).toLocaleString('fr-FR')} ${itemCurrency}`
                              return (
                                <li key={idx} className="flex gap-4 py-3 border-b border-white/10 last:border-0 last:pb-0 items-center">
                                  <div className="shrink-0 w-14 h-14 rounded-lg overflow-hidden bg-white/10 border border-white/10">
                                    {imageUrl ? (
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-white/50 text-xs mt-0.5">{qty} × {fmt(unitPrice)}</p>
                                  </div>
                                  <span className="text-white/90 font-medium tabular-nums whitespace-nowrap shrink-0">{fmt(lineTotal)}</span>
                                </li>
                              )
                            })}
                          </ul>
                          <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/10">
                            ...
                            <span className="text-cyan-300 font-semibold tabular-nums">
                              {(String(currency).toUpperCase() === 'CDF' || String(currency).toUpperCase() === 'FC') ? formatCurrencyCDF(order.total_amount ?? 0) : `${Number(order.total_amount ?? 0).toLocaleString('fr-FR')} ${currency}`}
                            </span>
                          </div>
                          {order.points_earned != null && (
                            <p className="text-white/50 text-xs mt-1">{order.points_earned} pts à gagner</p>
                          )}
                        </div>
                      )}
