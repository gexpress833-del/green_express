...
/**
 * Liste commandes + attribution livreur (admin ou secrétariat).
 * @param {{ sidebar: import('react').ReactNode, spaceLabel: string, basePath: string, showLoyaltyPoints?: boolean }} props
 */
  ...
  return (
    <section className="page-section page-section--admin-tight min-h-screen bg-[#0b1220] text-white">
      <header className="mb-10 max-w-3xl">
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-cyan-400/70">{spaceLabel}</p>
        <h1
        ...
        <p className="mt-3 text-sm leading-relaxed text-white/55">
          {orderFilterId
            ...
            : 'Vue des commandes : statut, acheteur, détail des plats et attribution des livreurs.'}
        </p>
      </header>
        ...
        <main className="main-panel">
          {loading ? (
            ...
            <div className="card rounded-2xl border border-white/10 p-8 text-center">
              <p className="text-lg text-white/80">Aucune commande n°{orderFilterId} dans la liste.</p>
              <p className="mt-2 text-sm text-white/45">Vérifiez l’identifiant ou ouvrez la liste complète.</p>
              <Link
                href={basePath}
            ...
            <div className="space-y-6">
              ...
              {displayOrders.map((order) => {
                ...
                return (
                  <div key={order.id} className="order-card order-card--marketing card rounded-2xl p-5 sm:p-7">
                    <div className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0 space-y-2">
                        <h2 className="text-xl font-semibold tracking-tight text-white sm:text-[1.35rem]">
                          Commande n°{order.id}
                          {order.uuid && (
                            <span className="ml-1.5 font-normal text-white/40">· {order.uuid.substring(0, 8)}…</span>
                          )}
                        </h2>
                        <p className="text-[13px] leading-relaxed text-white/50">{formatDate(order.created_at)}</p>
                        {order.delivery_address && (
                          <p className="flex max-w-xl items-start gap-2 text-[13px] leading-relaxed text-white/60">
                            <span className="mt-0.5 shrink-0 opacity-80" aria-hidden>
                              📍
                            </span>
                    ...
                    {order.user && (
                      <div className="mt-5 rounded-xl border border-white/[0.09] bg-black/20 px-4 py-4 sm:px-5">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-300/75">Acheteur</p>
                            ...
                            {showLoyaltyPoints && (
                              <p className="mt-2 text-[13px] leading-relaxed text-white/55">
                                ...
                                <span className="ml-2 font-semibold tabular-nums text-amber-100">
                                  {order.user.points?.balance != null ? Number(order.user.points.balance) : 0} points
                                </span>
                      ...
                      <div className="mt-6 rounded-xl border border-white/[0.09] bg-black/15 px-4 py-5 sm:px-6">
                        ...
                        <ul className="m-0 list-none space-y-0 p-0">
                          {order.items.map((item, idx) => {
                            ...
                            const qty = isLikelyTotal
                              ? Math.max(1, Math.round(rawQty / unitPrice))
                            ...
                            return (
                              <li key={idx} className="border-b border-white/[0.07] py-5 last:border-0 last:pb-0 first:pt-0">
                                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between lg:gap-8">
                                  <div className="min-w-0 flex-1 space-y-2">
                                    ...
                                    <p className="text-[17px] font-semibold leading-snug tracking-tight text-white">{title}</p>
                        ...
                        <div className="mt-6 border-t border-white/[0.08] pt-5">
                          <dl className="order-total-box">
                            ...
                            <dd className="order-total-box__value">{formatOrderMoney(order.total_amount, currency)}</dd>
                          </dl>
                            ...
                            <p className="mt-3 text-[13px] leading-relaxed text-white/45">
                              {showLoyaltyPoints ? (
                                <>
                                  ...
                                  <span className="font-medium text-amber-200/90">{order.points_earned} points</span> après
                                  validation du code par le livreur.
                                </>
                              ) : (
                                <>
                                  Fidélité :{' '}
                                  <span className="font-medium text-amber-200/90">{order.points_earned} points</span> crédités
                                  après livraison validée.
                                </>
                              )}
                            </p>
                          )}
                    ...
                    {order.delivery_code && (
                      <div className="mt-6 rounded-lg border border-cyan-500/20 bg-cyan-500/[0.06] px-4 py-3">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-cyan-200/70">Code livraison</p>
                        <p className="mt-2 font-mono text-lg font-semibold tracking-wider text-cyan-100">{order.delivery_code}</p>
                      </div>
                    )}
                    ...
                    {order.delivery_code && (order.status === 'pending' || order.status === 'out_for_delivery') && (
                      <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-white/10 pt-5">
                        ...
                        <select
                            ...
                            apiRequest(`/api/orders/${order.id}/assign-livreur`, {
                              ...
                              .then((updated) => {
                                setOrders((prev) =>
                                  prev.map((o) =>
                                    o.id === order.id
                                      ? { ...o, livreur_id: updated.livreur_id, delivery_driver: updated.delivery_driver }
                                      : o
                                  )
                                )
                              })
                          ...
                          aria-label={`Attribuer un livreur à la commande ${order.id}`}
                        >
                        ...
                        {order.delivery_driver && (
                          <span className="text-white/60 text-sm">
                            Actuel : {order.delivery_driver.name || order.delivery_driver.email}
                          </span>
                        )}
