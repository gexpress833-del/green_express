...
const COL_STYLES = {
  th: [
    ...
    { background: 'rgba(6, 182, 212, 0.25)', color: '#fff' },
    ...
    { background: 'rgba(245, 158, 11, 0.25)', color: '#fff' },
    { background: 'rgba(249, 115, 22, 0.25)', color: '#fff' },
    { background: 'rgba(100, 116, 139, 0.25)', color: '#fff' },
    { background: 'rgba(236, 72, 153, 0.25)', color: '#fff' },
  ],
  td: [
...
/**
 * Liste des demandes événementielles (API /api/admin/event-requests) — partagé admin / secrétariat.
 */
  ...
  return (
    <section className="page-section page-section--admin-tight min-h-screen bg-[#0b1220] text-white">
      <header className="mb-8">
        ...
        <p className="text-white/70 text-lg">Demandes de devis reçues depuis la page Service événementiel.</p>
      </header>
            ...
            <div className="card p-0 min-w-0 overflow-hidden">
              ...
              <div className="max-w-full overflow-hidden" style={{ isolation: 'isolate' }}>
                <div
                  ...
                  tabIndex={0}
                  title="Défiler horizontalement : molette (ou Shift+molette) ou barre de défilement"
                  ...
                  <table className="text-left border-collapse" style={{ width: 'max-content', minWidth: '1100px' }}>
                    <thead>
                      <tr className="text-sm font-semibold">
                        <th
                          className="py-3 px-4 border-b border-r border-white/15 text-center min-w-[120px]"
                          style={COL_STYLES.th[8]}
                        >
                          Action
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15" style={COL_STYLES.th[0]}>
                          Date demande
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15" style={COL_STYLES.th[1]}>
                          Contact
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15" style={COL_STYLES.th[2]}>
                          Type
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15 text-right" style={COL_STYLES.th[3]}>
                          Date événement
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15 text-right" style={COL_STYLES.th[4]}>
                          Personnes
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15" style={COL_STYLES.th[5]}>
                          Budget
                        </th>
                        <th className="py-3 px-4 border-b border-r border-white/15" style={COL_STYLES.th[6]}>
                          Statut
                        </th>
                        <th className="py-3 px-4 border-b border-white/15 min-w-[200px]" style={COL_STYLES.th[7]}>
                          Message
                        </th>
                      </tr>
                    </thead>
                      ...
                      {list.map((row) => {
                        ...
                        return (
                          <tr key={row.id} className="border-b border-white/10 hover:bg-white/[0.03] transition-colors">
                            <td
                              className="py-3 px-4 text-center align-top min-w-[120px] border-r border-white/10"
                              style={COL_STYLES.td[8]}
                            >
                              <button
                                type="button"
                            ...
                            <td
                              className="py-3 px-4 border-r border-white/10 text-sm whitespace-nowrap"
                              style={COL_STYLES.td[0]}
                            >
                            ...
                            <td className="py-3 px-4 border-r border-white/10 text-sm align-top" style={COL_STYLES.td[1]}>
                              {hasContact ? (
                                <>
                                  ...
                                  {contactEmail && (
                                    <a
                                      ...
                                      className="text-xs block truncate max-w-[200px] hover:underline"
                                      title={contactEmail}
                                      style={{ color: 'inherit' }}
                                    >
                                      {contactEmail}
                                  ...
                                  {row.contact_phone && (
                                    <a
                                      ...
                                      className="text-xs block mt-0.5 hover:underline"
                                      title={row.contact_phone}
                                  ...
                                  {row.user && (row.contact_name || row.contact_email) && (
                                    <span className="text-xs block mt-1" style={{ opacity: 0.7 }}>
                                      Compte lié
                                    </span>
                                  )}
                                </>
                                ...
                                <span className="italic" style={{ opacity: 0.8 }}>
                                  Ancienne demande (sans contact)
                                </span>
                              )}
                            </td>
                            <td className="py-3 px-4 border-r border-white/10 font-medium text-sm" style={COL_STYLES.td[2]}>
                              {row.event_type}
                            </td>
                            <td
                              className="py-3 px-4 border-r border-white/10 text-sm whitespace-nowrap text-right tabular-nums"
                              style={COL_STYLES.td[3]}
                            >
                              {formatDate(row.event_date)}
                            </td>
                            <td
                              className="py-3 px-4 border-r border-white/10 text-right tabular-nums font-medium text-sm"
                              style={COL_STYLES.td[4]}
                            >
                              {row.guest_count}
                            </td>
                            <td className="py-3 px-4 border-r border-white/10 text-sm" style={COL_STYLES.td[5]}>
                              {row.budget || '—'}
                            </td>
                            <td className="py-3 px-4 border-r border-white/10" style={COL_STYLES.td[6]}>
                              <span
                                className={`badge ${row.status === 'pending' ? 'badge-warning' : row.status === 'contacted' ? 'badge-success' : 'badge-error'}`}
                              >
                                {row.status === 'pending'
                                  ? 'En attente'
                            ...
                            <td
                              className="py-3 px-4 text-sm min-w-[200px] max-w-[280px] align-top border-r border-white/10 break-words"
                              style={COL_STYLES.td[7]}
                              title={row.message || ''}
                            >
                              {row.message || '—'}
                            </td>
      ...
      {treating && (
        ...
        >
          <div
            className="bg-[#0b1220] border border-white/20 rounded-xl max-w-lg w-full p-6 shadow-xl"
